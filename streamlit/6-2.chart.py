...
chart = alt.Chart(df_melted, title='일별 팀 매출 비교').mark_bar().encode(x='date', y='sales', color='teams', strokeDash='teams').properties(width=650, height=350)
    
    
text = alt.Chart(df_melted).mark_text(dx=0, dy=0, color='black').encode(x='date', y='sales', detail='teams', text=alt.Text('sales:Q'))

st.altair_chart(chart + text, use_container_width=True)  # use_container_width=True 가로로 화면에 채워 줌
...
fig = px.pie(medal, names = 'nation', values = 'gold', title = '올림픽 양궁 금메달 현황', hole = 0.3)
...
fig = px.bar(medal, x='nation', y=['gold', 'silver', 'bronze'], text_auto=True, title='올림픽 양궁 메달 현황') #text_auto=True 값 표시 여부
